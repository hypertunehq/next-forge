...
Go to Hypertune and create a new project using the [Next forge template](https://app.hypertune.com/?new_project=1&new_project_template=next-forge). Then go the settings page of the project and copy the main token.
...
Add `analyze` and `build` scripts to the `package.json` file for the Feature flags package which call `hypertune`. 
...
Then run the code generation using the following command.
...
Hypertune adds `Flag` suffix to all feature flags, so you will need to update all flag usages to use this new name (e.g. `showBetaFeature` => `showBetaFeatureFlag`). 
